...
//	debugger
	...
	//判断此用户是否存在于数据库中
	var url = 'login.do';//设定异步通信使用的url;
	var params = {'username':userName,'password':userPwd};//设置异步通信要上传的数据
	$.post(url,params,function(result){//post方法异步通信服务器，获取服务器返回的登陆结果
		if(result.state==1){   //用户校验成功，跳转到主页面
			location.href='indexUI.do';//跳转主页面
		}else{
			$('#errorMessage').parent().parent().css('display','block');//
			$('#errorMessage').text(result.message);//登陆失败，显示服务器返回的消息
		}
	})
}
</script>
